package concurrency.concurrency.barriers.after; /**
...
class Shopper extends Thread {
    ...
    public void run() {
        if (this.getName().contains("Olivia")) {
            ...
            // Will enforce Olivia threads are done first, before the barrier
            try {
                fistBump.await();
            ...
            // will enforce the Baron threads are done after the barrier, so wait for all olivia threads to be done
            try {
                fistBump.await();
            } catch (InterruptedException | BrokenBarrierException e) {
                e.printStackTrace();
