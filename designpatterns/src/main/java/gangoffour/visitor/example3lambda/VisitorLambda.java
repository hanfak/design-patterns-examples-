package gangoffour.visitor.example3lambda;
...
public class VisitorLambda {
  ...
  static final Function<Object, Double> areaVisitor = new LambdaVisitor<Double>()
  ...
  static final Function<Object, Double> perimeterVisitor = new LambdaVisitor<Double>()
...
//    double totalArea = figures.stream().map( areaVisitor ).reduce( 0.0, Double::sum);
    double totalArea = figures.stream().map( areaVisitor ).count();
